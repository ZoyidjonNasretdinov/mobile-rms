...
export default function MenuItemModal({
    ...
    try {
      ...
      if (itemId) {
        const [itemRes, recipeRes] = await Promise.all([
          ...
          axios.get(`${API_BASE_URL}/menu/items/${itemId}/recipe`, { headers }),
        ]);
        const item = itemRes.data.find((i: any) => i._id === itemId);
        if (item) {
          setForm({
            name: item.name,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (itemId) {
        await axios.patch(`${API_BASE_URL}/menu/items/${itemId}`, data, {
          headers,
        });
      } else {
        ...
        currentItemId = res.data._id;
      }

      // Save Recipe
      if (currentItemId) {
        await axios.post(
          `${API_BASE_URL}/menu/items/${currentItemId}/recipe`,
          {
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent>
      <View style={styles.modalOverlay}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
              ...
              <View style={[styles.row, styles.availabilityRow]}>
                ...
                <Switch
                  ...
                  thumbColor={
                    form.isAvailable ? colors.primary : colors.secondary
                  }
                />
              </View>
        ...
        {/* Product Selection Modal (Nested) */}
        <Modal visible={showProductModal} transparent animationType="slide">
          <View style={styles.nestedModalOverlay}>
            <View
              style={[
                styles.nestedModalContent,
                { backgroundColor: colors.card },
              ]}
            >
